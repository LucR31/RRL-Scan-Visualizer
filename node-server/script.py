...
names = ["milliseconds",
         ...
         'RRL1WTO:IST:2',
         '[degC_2]',
         'RRL1WTU:IST:2','[degC_3]','MNC3:IST:2','[uA]','RRL1A:I3:2','[µA_2]','EICI:IST:2','[µA_3]','s']
df = pd.read_csv(dir + 'tendis_20250707_103559.txt', sep='\t', header=None, 
                 names=names,
                 encoding='unicode_escape')
df = df.drop(['[degC_1]','[kV]','[µA_1]','[degC_2]','[degC_3]','[µA_2]','[µA_3]','s','[uA]','[mm]'], axis=1)
df = df.iloc[1:]
...
def fill_scan_name_col(df:pd.DataFrame,
                       list_files:list,
                       rrl_column:str,
                       max_val = 4550)-> pd.DataFrame:
    df['scan'] = 0
    count,indicator = 0,0
